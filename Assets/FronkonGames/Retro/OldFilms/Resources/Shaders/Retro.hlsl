...
// HSV conversion functions are already defined in Macros.hlsl
...
inline float3 ColorAdjust(float3 pixel, float contrast, float brightness, float hue, float gamma, float saturation)
{
  ...
  gamma = max(gamma, 0.001); // Prevent division by zero
  
  pixel = max(0.0, (pixel - (float3)0.5) * contrast + (float3)0.5 + brightness);

  // Only perform HSV conversion if hue adjustment is needed
  if (abs(hue) > 0.001)
  {

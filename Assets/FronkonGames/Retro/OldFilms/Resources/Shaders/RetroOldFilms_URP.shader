  ...
  SubShader
  {
    ...
    Pass
    {
      ...
      float4 frag(const RetroVaryings input) : SV_Target 
      {
        ...
        // Move frame.
        uv.x += _MoveFrame.x * (cos(_EffectTime.y * 3.0) * sin(_EffectTime.y * 12.0 + 0.25));
        uv.y += _MoveFrame.y * (sin(_EffectTime.y * 1.0 + 0.5) * cos(_EffectTime.y * 15.0 + 0.25));

        // Jump frame.
        uv += float2(min(frac(Rand(_EffectTime.y)) - _JumpFrame, 0.0), 0.0);
        ...
        if (_Scratches > 0.0 && _RandomValue.z < _Scratches)
        {
          ...
          if (d < 0.4)
          {
            ...
            float scratch = 0.5 + (sin(((uv.x * xPeriod + uv.y * yPeriod + turbulence)) * PI + _EffectTime.y) * 0.5);
        ...
        // Blink.
        pixel.rgb *= (1.0 - _BlinkStrength) + _BlinkStrength * sin(_BlinkSpeed * _EffectTime.y);
